...
function DoorConfigurator() {
  ...
  function createEmptyDoor() {
    return {
      ...
      operator: 'NONE',
      operatorAccessories: [],  // Array of accessory part numbers
...
function DesignStep({ door, colors, panelDesigns, config, onChange }) {
  ...
  // TX500 = white+black only, TX450-20/TX500-20 = white only per rulebook
  const colorMap = {
    'KANATA': 'KANATA',
    'CRAFT': 'CRAFT',
    'TX380': 'COMMERCIAL',
            ...
            <div key={brand} className="mb-2">
              <button
                ...
                className={`w-full p-3 rounded-lg border-2 text-left transition-all flex items-center justify-between ${
                  selectedInBrand ? 'border-odc-500 bg-odc-50' : 'border-gray-200 hover:border-gray-300'
                }`}
              >
                <div>
            ...
            return (
              <div key={brand} className="mb-2">
                <button
                  ...
                  className={`w-full p-3 rounded-lg border-2 text-left transition-all flex items-center justify-between ${
                    selectedCount > 0 ? 'border-odc-500 bg-odc-50' : 'border-gray-200 hover:border-gray-300'
                  }`}
                >
                  <div>
                    <h4 className="font-medium text-gray-900 text-sm">{brand}</h4>
...
function CustomerSelector({ selectedCustomer, onCustomerChange }) {
  ...
  return (
    <div className="relative min-w-[260px]" ref={containerRef}>
      ...
      {!selectedCustomer && (
        <p className="mt-1 text-xs text-amber-500 font-medium">Please select a customer before generating a quote</p>
      )}
